...
class GPMongoDBFactoryConfig {
    ...
    @Autowired
    public MongoDbFactory gpMongoDBFactory(@Qualifier(
            value = "gpSpringMongoClient") MongoClient gpSpringMongoClient,
            @Qualifier(value = "gpSpringMongoProp") MongoProperties gpSpringMongoProp) {

        logger.debug("###################### GeoPlatform Experimental ::== "
                + "Building MongoDBFactory.\n");

        return new SimpleMongoDbFactory(gpSpringMongoClient,
                gpSpringMongoProp.getMongoDatabaseName(),
                gpSpringMongoProp.getUserCredential());
    }
