 ...
 */
public class SaveLayerHandler extends SaveTreeElementHandler {

    protected final static Logger logger = Logger.getLogger("SaveLayerHandler");

    //
    public SaveLayerHandler() {
        request.setElementType(GPTreeCompositeType.LEAF);
    }

    @Override
    public void saveTreeElement(MementoSaveDragDrop memento) {
        if (memento.getRefBaseElement() instanceof GPLayerBean) {
            doSave(memento);
        } else {
        ...
        GPClientCommandExecutor.executeCommand(
            ...
            public void onCommandSuccess(SaveDragAndDropResponse response) {
                ...
                LayoutManager.getInstance().getStatusMap().setStatus(
                        MementoPersistenceConstants.INSTANCE.GPDNDListener_statusSaveLayerDDSuccessText(),
            ...
            public void onCommandFailure(Throwable exception) {
                if (exception.getCause() instanceof GPSessionTimeout) {
                    ...
                    GeoPlatformMessage.errorMessage(
                            MementoPersistenceConstants.INSTANCE.GPDNDListener_saveLayerDDErrorTitleText(),
                            MementoPersistenceConstants.INSTANCE.GPDNDListener_saveLayerDDErrorBodyText());
                }
            }
