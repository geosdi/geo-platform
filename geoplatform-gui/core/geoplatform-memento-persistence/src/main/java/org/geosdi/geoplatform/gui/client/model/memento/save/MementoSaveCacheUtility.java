...
public class MementoSaveCacheUtility {
    ...
    private static boolean verifyCondition(GPBeanTreeModel element, IMemento<ISave> memento) {
        boolean condition = false;
        if (element instanceof AbstractFolderTreeNode) {
            condition = verifyFolderCondition((AbstractFolderTreeNode) element, memento);
        } else if (element instanceof GPLayerTreeModel) {
            condition = verifyLayerCondition((GPLayerTreeModel) element, memento);
        }
        return condition;
    }
    ...
    private static boolean verifyFolderCondition(AbstractFolderTreeNode folder, IMemento<ISave> memento) {
        boolean condition = false;
        if (memento instanceof MementoSaveAddedFolder
                && ((MementoSaveAddedFolder) memento).getAddedFolder().getRefBaseElement().equals(folder)) {
            condition = true;
        }
        return condition;
    }
