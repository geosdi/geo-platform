 ...
 */
@Singleton
public class GPLoginAccessManager extends GeoPlatformContentPanel {
...
//        this.getParent().getElement().getStyle().setDisplay(Style.Display.NONE);
//        this.getElement().getStyle().setDisplay(Display.NONE);
        ...
        timer = new Timer() {
            ...
            public void run() {
                progres += 0.1;
                progressBar.setRunProgress(progres, loadingMessage);
                if (progres > 1.0) {
                    progressBar.setRunProgress(0.0, "");
                }
            }
    ...
    public void hideProgressBar(boolean isSuccess) {
        timer.cancel();
        this.progressBar.setRunProgress(1.0, "Done");
