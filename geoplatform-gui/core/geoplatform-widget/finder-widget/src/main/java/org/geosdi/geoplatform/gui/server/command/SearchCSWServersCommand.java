    ...
    public SearchCSWServersResponse execute(SearchCSWServersRequest request, HttpServletRequest httpServletRequest) {
        ...
        try {
            ...
            if (serversCount == 0) {
                ...
                searchServers = serverList.stream().filter(Objects::nonNull).map(SearchCSWServersCommand::convertServerDTO).collect(toCollection(ArrayList::new));
            }
            return new SearchCSWServersResponse(
                    new BasePagingLoadResult<GPCSWServerBeanModel>(searchServers, request.getConfig().getOffset(),
                            serversCount));
        } catch (Exception ex) {
            ex.printStackTrace();
