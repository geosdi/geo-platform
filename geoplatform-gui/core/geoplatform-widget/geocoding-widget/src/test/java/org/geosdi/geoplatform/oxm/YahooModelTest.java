    ...
    public void testGeocoding() {
        try {
            File source = new File(
                    of(new File(".").getCanonicalPath(), "src", "test", "resources", "yahooGeocodeExample").collect(
                            joining(File.separator, "", ".xml")));
            if (!source.canRead()) {
    ...
    public void testReverseGeocoding() {
        try {
            File source = new File(of(new File(".").getCanonicalPath(), "src", "test", "resources",
                    "yahooReverseGeocodeExample").collect(joining(File.separator, "", ".xml")));
            if (!source.canRead()) {
                throw new IllegalArgumentException("Source path " + source.getPath() + " is not valid");
            }
            GPYahooGeocodeRoot geocode = (GPYahooGeocodeRoot) geocoderYahooJaxbMarshaller.unmarshal(source);
