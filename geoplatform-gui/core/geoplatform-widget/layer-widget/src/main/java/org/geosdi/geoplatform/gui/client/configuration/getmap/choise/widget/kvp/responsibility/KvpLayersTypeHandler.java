 ...
 */
public class KvpLayersTypeHandler extends KvpUrlTypeHandler {
    
    public KvpLayersTypeHandler() {
        super.setSuccessor(new KvpSrsTypeHandler());
    }
    ...
    protected Boolean canCheckKvpUrl(String url) {
        return (!GPRegEx.RE_LAYERS.test(url)) ? notifyMessage() : Boolean.FALSE;
    }
    
    @Override
    protected String getNotificationMessage() {
        return LayerModuleMessages.INSTANCE.
                LoadWmsGetMapFromUrlWidget_suggestionCheckGetMapLayersMessage(
                        GetMap.LAYERS.toString());
    }
