 ...
 */
public class KvpRequestTypeHandler extends KvpUrlTypeHandler {

    public KvpRequestTypeHandler() {
        super.setSuccessor(new KvpVersionTypeHandler());
    }
    ...
    protected Boolean canCheckKvpUrl(String url) {
        return (!GPRegEx.RE_REQUEST.test(url)) ? notifyMessage() : Boolean.FALSE;
    }

    @Override
    protected String getNotificationMessage() {
        return LayerModuleMessages.INSTANCE.
                LoadWmsGetMapFromUrlWidget_suggestionCheckGetMapRequestMessage(
                        GetMap.REQUEST.toString());
    }
