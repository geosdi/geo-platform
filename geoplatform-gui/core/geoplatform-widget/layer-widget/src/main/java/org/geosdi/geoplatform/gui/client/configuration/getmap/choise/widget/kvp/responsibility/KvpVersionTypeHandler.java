 ...
 */
public class KvpVersionTypeHandler extends KvpUrlTypeHandler {
    
    public KvpVersionTypeHandler() {
        super.setSuccessor(new KvpLayersTypeHandler());
    }
    ...
    protected Boolean canCheckKvpUrl(String url) {
        return (!GPRegEx.RE_VERSION.test(url)) ? notifyMessage() : Boolean.FALSE;
    }
    
    @Override
    protected String getNotificationMessage() {
        return LayerModuleMessages.INSTANCE.
                LoadWmsGetMapFromUrlWidget_suggestionCheckGetMapVersionMessage(
                        GetMap.VERSION.toString());
    }
