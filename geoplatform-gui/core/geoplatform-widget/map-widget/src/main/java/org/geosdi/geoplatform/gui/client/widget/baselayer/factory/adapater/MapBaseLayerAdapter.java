...
public class MapBaseLayerAdapter implements GPBaseLayerAdapter {
    ...
    @Override
    public Map<GPBaseLayerValue, GPBaseLayer> adapt(Map<GPBaseLayerValue, GPBaseLayerCreator> baseLayerMap) {
        if (baseLayerMap == null) {
            throw new IllegalArgumentException("The Map passed must not be null");
        }
        Map<GPBaseLayerValue, GPBaseLayer> adapterMap = Maps.newHashMap();
        for (Map.Entry<GPBaseLayerValue, GPBaseLayerCreator> entry : baseLayerMap.entrySet()) {
    ...
    @Override
    public GPBaseLayer adaptBaseLayer(GPBaseLayerValue key, Layer value) {
        return new GPBaseLayer(value, key.getIcon(), new Projection(key.getProjectionCode()),
                key);
    }
}
