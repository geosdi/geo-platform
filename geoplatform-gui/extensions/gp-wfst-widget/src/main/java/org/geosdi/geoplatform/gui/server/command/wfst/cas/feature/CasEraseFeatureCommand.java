 ...
 */
@Lazy(true)
@Component(value = "command.wfst.cas.feature.CasEraseFeatureCommand")
@Profile(value = "gs_cas")
public class CasEraseFeatureCommand implements
        GPCommand<CasEraseFeatureRequest, EraseFeatureResponse> {

    private static final Logger logger = LoggerFactory.getLogger(
            CasEraseFeatureCommand.class);
    //
    @Autowired
    private CasTransactionService gpCasTransactionService;

    @Override
    public EraseFeatureResponse execute(CasEraseFeatureRequest request, HttpServletRequest httpServletRequest) {
        ...
        try {
            ...
            boolean result = this.gpCasTransactionService.transactionDelete(url, request.getTypeName(),
                    request.getFid(), EMPTY_MAP);
