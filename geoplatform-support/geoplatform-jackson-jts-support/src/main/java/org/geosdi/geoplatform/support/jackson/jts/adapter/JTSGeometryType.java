...
public enum JTSGeometryType implements GPJTSGeometryType {
     ...
     * @param theType
     */
    JTSGeometryType(@Nonnull(when = NEVER) String theType) {
     ...
     */
    public static JTSGeometryType forType(@Nonnull(when = NEVER) String theType) {
        checkArgument((theType != null) && !(theType.trim().isEmpty()),
                "The Parameter theType must not be null or an Empty String.");
        Optional<JTSGeometryType> optional = stream(JTSGeometryType.values())
