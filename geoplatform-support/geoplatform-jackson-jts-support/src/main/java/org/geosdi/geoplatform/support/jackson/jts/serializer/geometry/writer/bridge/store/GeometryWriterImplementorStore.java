...
public class GeometryWriterImplementorStore implements GPGeometryWriterImplementorStore {
    ...
    static {
        geometryWriterImplementors = stream(finder.getValidImplementors().spliterator(), FALSE)
                ...
                .collect(toMap(i -> i.getKey(), identity()));
     ...
     */
    @Override
    public GeometryWriterImplementor getImplementorByKey(GPImplementor.GPImplementorKey key) throws Exception {
     ...
     */
    @Override
    public GeometryWriterImplementor<? extends GPJTSGeometryAdapter, ? extends GeoJsonObject> getImplementorByKey(Class<?> key) throws Exception {
        checkArgument(key != null, "The Key must not be null.");
