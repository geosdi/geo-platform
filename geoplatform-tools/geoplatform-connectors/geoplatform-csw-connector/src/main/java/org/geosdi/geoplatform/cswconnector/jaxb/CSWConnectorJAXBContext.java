 ...
 */
public class CSWConnectorJAXBContext {

    private static GPConnectorJAXBContext instance;

    static {
        try {
            instance = new CSWJAXBContext(
                    CSWContextServiceProvider.loadContextPath());
        } catch (JAXBException e) {
            LoggerFactory.getLogger(CSWConnectorJAXBContext.class).error(
                    "Failed to Initialize JAXBContext : @@@@@@@@@@@@@@@@@ " + e);
        }
    }

    private CSWConnectorJAXBContext() {
    }

    public static GPConnectorJAXBContext getInstance() {
        return instance;
    }
}
