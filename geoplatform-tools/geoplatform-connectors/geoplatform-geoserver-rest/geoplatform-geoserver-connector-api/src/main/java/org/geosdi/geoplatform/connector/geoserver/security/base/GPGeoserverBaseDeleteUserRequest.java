...
import org.geosdi.geoplatform.connector.geoserver.request.security.users.base.GeoserverBaseDeleteUserRequest;
 ...
 */
public abstract class GPGeoserverBaseDeleteUserRequest<R extends GeoserverBaseDeleteUserRequest> extends GPJsonDeleteConnectorRequest<Boolean, R> implements GeoserverBaseDeleteUserRequest<R> {
     ...
     */
    protected GPGeoserverBaseDeleteUserRequest(@Nonnull(when = NEVER) GPServerConnector theServerConnector, @Nonnull(when = NEVER) JacksonSupport theJacksonSupport) {
     ...
     */
    @Override
    public R withUser(@Nonnull(when = NEVER) String thUser) {
        this.user.set(thUser);
        return self();
    }
