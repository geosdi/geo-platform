...
public class GPGeoserverConnectionKey implements IGPGeoserverConnectionKey {
     ...
     */
    GPGeoserverConnectionKey(@Nonnull(when = When.NEVER) String theConnectionKey, @Nullable String connectionDescription,
            @Nonnull(when = When.NEVER) GPGeoserverConnectionKeyLevel theLevel, @Nonnull(when = When.NEVER) String theType,
            boolean theRequired, @Nullable Object theDefaultValue) {
        Preconditions.checkArgument((theConnectionKey != null) && !(theConnectionKey.trim().isEmpty()), "The Parameter connectionKey must not be null or an empty string.");
        Preconditions.checkNotNull(theLevel, "The Parameter level must not be null");
