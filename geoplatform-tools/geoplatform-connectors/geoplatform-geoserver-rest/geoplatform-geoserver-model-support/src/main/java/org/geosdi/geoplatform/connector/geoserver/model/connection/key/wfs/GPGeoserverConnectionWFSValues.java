...
public enum GPGeoserverConnectionWFSValues implements IGPGeoserverConnectionKey {
    ...
    FILTER_COMPLIANCE(of("Filter compliance", "Level of compliance to WFS specification (0-low,1-medium,2-high)", user, "Integer", FALSE, WFSFilterCompliance.LOW)),
    ...
    AXIS_ORDER_FILTER(of("Axis Order Filter", "Indicates axis order used by the remote WFS server for filters. It applies only to WFS 1.x.0 servers. Default is the same as AXIS_ORDER", advanced, "String", FALSE, WFSAxisOrder.COMPLIANT)),
    ...
    AXIS_ORDER(of("Axis Order", "Indicates axis order used by the remote WFS server in result coordinates. It applies only to WFS 1.x.0 servers. Default is Compliant", advanced, "String", FALSE, WFSAxisOrder.COMPLIANT)),
    WFS_STRATEGY(of("WFS Strategy", "Override wfs stragegy with either cubwerx, ionic, mapserver, geoserver, strict, nonstrict or arcgis strategy.", user, "String", FALSE, WFSStrategy.auto)),
     ...
     * @param theKey
     */
    GPGeoserverConnectionWFSValues(@Nonnull(when = When.NEVER) IGPGeoserverConnectionKey theKey) {
