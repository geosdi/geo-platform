...
import org.geosdi.geoplatform.connector.geoserver.model.connection.key.db.postgis.jndi.GPGeoserverConnectionPostGISJndiValues;
...
public interface IGPPostgisJndiDatastoreBuilder extends IGPBasePostgisDatastoreBodyBuilder<IGPPostgisJndiDatastoreBuilder> {
     ...
     */
    IGPPostgisJndiDatastoreBuilder withJndiReferenceName(@Nonnull(when = When.NEVER) String theJndiReferenceName);
    ...
    class GPPostgisJndiDatastoreBuilder extends GPBasePostgisDatastoreBodyBuilder<IGPPostgisJndiDatastoreBuilder> implements IGPPostgisJndiDatastoreBuilder {
        ...
        static {
            requiredValues = GPGeoserverConnectionPostGISJndiValues.requiredValues();
            defaultValues = GPGeoserverConnectionPostGISJndiValues.defaultValues();
        }
        ...
        GPPostgisJndiDatastoreBuilder() {
            super(GeoserverDatastoreType.POSTGIS_JNDI);
        }
         ...
         */
        @Override
        public IGPPostgisJndiDatastoreBuilder withJndiReferenceName(@Nonnull(when = When.NEVER) String theJndiReferenceName) {
            this.connectionParameters.compute(GPGeoserverConnectionPostGISJndiValues.JNDI_REFERENCE_MAME.getConnectionKey(), (k, v) -> ((theJndiReferenceName != null) && !(theJndiReferenceName.trim().isEmpty())) ? theJndiReferenceName : v);
            return self();
        }
        ...
        protected void checkConnectionParameters() throws Exception {
            for (IGPGeoserverConnectionKey key : requiredValues) {
                ...
                Preconditions.checkArgument((value != null) && !(value.trim().isEmpty()), "For the Key : " + key.getConnectionKey() + ", the value must not be null or an empty string.");
            }
        }
