...
public class GPGeoserverMasterPassword implements IGPGeoserverMasterPassword {
     ...
     */
    @JsonCreator
    public GPGeoserverMasterPassword(@Nonnull(when = When.NEVER) @JsonProperty(value = "oldMasterPassword") String theOldMasterPassword) {
        Preconditions.checkArgument((theOldMasterPassword != null) && !(theOldMasterPassword.trim().isEmpty()), "The Parameter oldMasterPassword must not be null or an empty value.");
