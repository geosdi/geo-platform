...
public class GPGeoserverUpdateMasterPassword extends GPGeoserverMasterPassword implements IGPGeoserverUpdateMasterPassword {
     ...
     */
    @JsonCreator
    public GPGeoserverUpdateMasterPassword(@Nonnull(when = When.NEVER) @JsonProperty(value = "oldMasterPassword") String theOldMasterPassword,
            @Nonnull(when = When.NEVER) @JsonProperty(value = "newMasterPassword") String theNewMasterPassword) {
        super(theOldMasterPassword);
        Preconditions.checkArgument((theNewMasterPassword != null) && !(theNewMasterPassword.trim().isEmpty()), "The Parameter newMasterPassword must not be null or an empty string.");
