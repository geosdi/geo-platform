...
/**
 * Common contract shared by the fluent, thread-safe CSW catalog requests whose per-thread configuration is held
 * in {@link ThreadLocal} state (GetRecords, GetRecordById). It hoists the {@link #clearState()} declaration to a
 * single place, and layers on top the {@link #execute} template, which owns the whole configure &rarr; terminal
 * &rarr; cleanup lifecycle so the per-thread state is released on <b>every</b> path (normal return <b>and</b>
 * exception) without relying on the caller to remember a {@code finally clearState()}.
 * <p>
 ...
 */
public interface CatalogStatefulRequest<T, SELF extends CatalogStatefulRequest<T, SELF>> extends IGPostConnectorRequest<T> {
     ...
     * issued inside the action (e.g. log {@code showRequestAsString()} then {@code getResponse()}) : the state
     * is released only once the action completes.
     *
     ...
     */
    default <R> R execute(@Nonnull(when = NEVER) CatalogTerminalFunction<? super SELF, R> action) throws Exception {
        if (action == null)
     ...
     * {@link #execute(CatalogTerminalFunction)}.
     *
    ...
    default <R> R execute(@Nonnull(when = NEVER) Consumer<? super SELF> configurator,
                          @Nonnull(when = NEVER) CatalogTerminalFunction<? super SELF, R> terminal) throws Exception {
