 ...
 */
public class GPWFSGetFeatureGml311Reader implements GPWFSGetFeatureReader<FeatureCollectionType> {

    private static final Logger logger = LoggerFactory.getLogger(GPWFSGetFeatureGml311Reader.class);
    //
     ...
     */
    @Override
    public FeatureCollectionType read(@Nonnull(when = NEVER) InputStream inputStream) throws Exception {
        checkArgument(inputStream != null, "The Parameter inputStream must not be null.");
        logger.debug("##############################Executing {}#read.", this);
        return jaxbContextBuilder.unmarshal(new InputStreamReader(inputStream, UTF_8), FeatureCollectionType.class);
    }
