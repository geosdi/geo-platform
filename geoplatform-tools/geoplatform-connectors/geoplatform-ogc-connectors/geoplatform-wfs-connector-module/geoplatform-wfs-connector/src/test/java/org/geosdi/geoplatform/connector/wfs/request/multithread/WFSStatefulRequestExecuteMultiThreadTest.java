 ...
 * on a <b>single, shared</b> {@code GetFeature} request driven concurrently by many <b>virtual threads</b> -
 * the exact scenario that motivates the template : the {@code execute} pattern is meant to be used <b>only
 * when the same request instance is shared/reused across several threads</b>, because a long-lived shared
 * instance would otherwise retain each thread's last {@code ThreadLocal} configuration. (When a fresh request
 * is created per call, as the production services do, the state dies with the instance and {@code execute} is
 * not needed.)
 * <p>
 * It asserts three properties : (a) under concurrency each thread always sees its own configuration inside the
 * action (thread-safety is preserved), and after every {@code execute} its per-thread state has been released;
 * (b) the state is released even when the action throws (the {@code finally clearState()} contract); (c) the
 * single-argument overload releases the state too. No network call is performed : the action relies on
 * {@link WFSGetFeatureRequest#showRequestAsString()}, which only marshals the current per-thread state.
 *
 * @author Giuseppe La Scaleia - CNR IMAA geoSDI Group
 * @email giuseppe.lascaleia@geosdi.org
 */
@FixMethodOrder(MethodSorters.NAME_ASCENDING)
public class WFSStatefulRequestExecuteMultiThreadTest {

    private static final Logger logger = LoggerFactory.getLogger(WFSStatefulRequestExecuteMultiThreadTest.class);
    //
    ...
    public void a_executeIsThreadSafeAndAlwaysReleasesStateAcrossVirtualThreads() throws Exception {
        WFSGetFeatureRequestState state = (WFSGetFeatureRequestState) sharedRequest;
        ...
        range(0, THREADS).forEach(index -> Thread.startVirtualThread(() -> {
            try {
                ...
                BigInteger maxFeatures = BigInteger.valueOf(index + 1L);
                startSignal.await();
                for (int i = 0; i < ITERATIONS; i++) {
                    String requestAsString = sharedRequest.execute(
                            r -> r.withTypeName(typeName)
                                    .withResultType(resultType)
                                    .withMaxFeatures(maxFeatures)
                                    .withFeatureIDs(asList(featureId)),
                            WFSGetFeatureRequest::showRequestAsString);
                    ...
                    checkArgument(requestAsString.contains(featureId),
                            "Cross-thread state leak on task %s : expected featureId [%s] but was [%s].",
                    ...
                    checkArgument(state.getFeatureIDs() == null,
                            "execute did not release featureIDs after the action on task %s.", index);
                    checkArgument(state.getMaxFeatures() == null,
                            "execute did not release maxFeatures after the action on task %s.", index);
    ...
    public void b_executeReleasesStateWhenTheActionThrows() throws Exception {
        ...
        try {
            sharedRequest.execute(
                    r -> r.withTypeName(typeName).withMaxFeatures(BigInteger.TEN),
                    r -> {
                        throw boom;
                    });
        ...
        assertNull("execute must release maxFeatures even when the action throws.", state.getMaxFeatures());
    }
    ...
    public void c_singleArgumentExecuteReleasesStateAfterTheAction() throws Exception {
        ...
        String requestAsString = sharedRequest.execute(r -> {
            r.withTypeName(typeName)
                    .withResultType(RESULTS.value())
                    .withMaxFeatures(BigInteger.ONE)
                    .withFeatureIDs(asList("fid_single"));
        ...
        assertNull("the single-argument execute must release the state after the action.", state.getTypeName());
        assertNull("the single-argument execute must release maxFeatures after the action.", state.getMaxFeatures());
    }
}