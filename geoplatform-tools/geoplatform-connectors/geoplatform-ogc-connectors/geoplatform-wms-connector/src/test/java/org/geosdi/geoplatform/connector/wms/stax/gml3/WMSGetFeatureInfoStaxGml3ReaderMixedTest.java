...
public class WMSGetFeatureInfoStaxGml3ReaderMixedTest {

    private static final Logger logger = LoggerFactory.getLogger(WMSGetFeatureInfoStaxGml3ReaderMixedTest.class);
    //
    ...
    public void a_wmsGetFeatureInfoStaxGml3ReaderTest() throws Exception {
        FeatureCollection featureCollection = wmsGetFeatureInfoStaxGml3Reader.read(file);
        logger.info("#######################FEATURE_COLLECTION_GML3 : {}\n", featureCollection);
        JACKSON_SUPPORT.getDefaultMapper().writeValue(new File(of(new File(".").getCanonicalPath(), "target", "FeatureCollectionGml3MIXED")
                .collect(joining(separator, "", ".json"))), featureCollection);
    }

    @Test
    public void b_wmsGetFeatureInfoStaxGml3ReaderTest() throws Exception {
        GPWMSFeatureStore wmsFeatureStore = wmsGetFeatureInfoStaxGml3Reader.readAsStore(file);
        ...
        JACKSON_SUPPORT.getDefaultMapper().writeValue(new File(of(new File(".").getCanonicalPath(), "target", "Store_MIXED_FEATURES_GML3")
