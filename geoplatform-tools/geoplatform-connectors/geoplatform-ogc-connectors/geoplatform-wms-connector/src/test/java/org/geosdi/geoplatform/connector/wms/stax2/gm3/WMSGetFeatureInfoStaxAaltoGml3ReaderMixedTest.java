...
public class WMSGetFeatureInfoStaxAaltoGml3ReaderMixedTest {

    private static final Logger logger = LoggerFactory.getLogger(WMSGetFeatureInfoStaxAaltoGml3ReaderMixedTest.class);
    //
    ...
    public void a_wmsGetFeatureInfoStaxGml3ReaderTest() throws Exception {
        FeatureCollection featureCollection = wmsGetFeatureInfoAaltoGml3Reader.read(file);
        logger.info("#######################FEATURE_COLLECTION_GML3 : {}\n", featureCollection);
        JACKSON_SUPPORT.getDefaultMapper().writeValue(new File(of(new File(".").getCanonicalPath(), "target", "FeatureCollectionAaltoGml3MIXED")
                .collect(joining(separator, "", ".json"))), featureCollection);
    }

    @Test
    public void b_wmsGetFeatureInfoStaxGml3ReaderTest() throws Exception {
        GPWMSFeatureStore wmsFeatureStore = wmsGetFeatureInfoAaltoGml3Reader.readAsStore(file);
        ...
        JACKSON_SUPPORT.getDefaultMapper().writeValue(new File(of(new File(".").getCanonicalPath(), "target", "AaltoStore_MIXED_FEATURES_GML3")
