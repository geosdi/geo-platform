 ...
 */
package org.geosdi.geoplatform.connector.wms.stax2.multithread.aalto;
 ...
 */
public class WMSGetFeatureInfoMultiThreadAaltoReaderTest extends GPWMSGetFeatureMultiThreadTest {

    private static final Logger logger = LoggerFactory.getLogger(WMSGetFeatureInfoMultiThreadAaltoReaderTest.class);
    ...
    public void wmsGetFeatureInfoMultiThreadAaltoReaderTest() throws Exception {
        ...
        fromIterable(files)
                .map(f -> new Thread(new WMSGetFeatureInfoAaltoReaderTask(f, startSignal, doneSignal, counter)))
    ...
    static class WMSGetFeatureInfoAaltoReaderTask implements Runnable {

        private static final Logger logger = LoggerFactory.getLogger(WMSGetFeatureInfoAaltoReaderTask.class);
        //
         ...
         * @param theDoneSignal
         */
        WMSGetFeatureInfoAaltoReaderTask(@Nonnull(when = NEVER) String theFileName, @Nonnull(when = NEVER) CountDownLatch theStartSignal,
        ...
        public void run() {
            try {
                ...
                logger.info("#######################FEATURE_COLLECTION : \n{}\n for File : {}\n", JACKSON_SUPPORT.getDefaultMapper()
                        .writeValueAsString(wmsGetFeatureInfoAaltoReader.read(new File(fileName))), fileName);
