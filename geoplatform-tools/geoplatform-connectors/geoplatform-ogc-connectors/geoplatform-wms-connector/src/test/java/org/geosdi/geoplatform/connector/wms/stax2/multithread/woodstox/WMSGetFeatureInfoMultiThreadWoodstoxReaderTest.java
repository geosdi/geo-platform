 ...
 */
package org.geosdi.geoplatform.connector.wms.stax2.multithread.woodstox;

import org.geosdi.geoplatform.connector.wms.stax.multithread.GPWMSGetFeatureMultiThreadTest;
 ...
 */
public class WMSGetFeatureInfoMultiThreadWoodstoxReaderTest extends GPWMSGetFeatureMultiThreadTest {

    private static final Logger logger = LoggerFactory.getLogger(WMSGetFeatureInfoMultiThreadWoodstoxReaderTest.class);
    ...
    public void wmsGetFeatureInfoMultiThreadAaltoReaderTest() throws Exception {
        ...
        fromIterable(files)
                .map(f -> new Thread(new WMSGetFeatureInfoWoodstoxReaderTask(f, startSignal, doneSignal, counter)))
    ...
    static class WMSGetFeatureInfoWoodstoxReaderTask implements Runnable {

        private static final Logger logger = LoggerFactory.getLogger(WMSGetFeatureInfoWoodstoxReaderTask.class);
        //
         ...
         * @param theDoneSignal
         */
        WMSGetFeatureInfoWoodstoxReaderTask(@Nonnull(when = NEVER) String theFileName, @Nonnull(when = NEVER) CountDownLatch theStartSignal,
        ...
        public void run() {
            try {
                ...
                logger.info("#######################FEATURE_COLLECTION : \n{}\n for File : {}\n", JACKSON_SUPPORT.getDefaultMapper()
                        .writeValueAsString(wmsGetFeatureInfoWoodstoxReader.read(new File(fileName))), fileName);
