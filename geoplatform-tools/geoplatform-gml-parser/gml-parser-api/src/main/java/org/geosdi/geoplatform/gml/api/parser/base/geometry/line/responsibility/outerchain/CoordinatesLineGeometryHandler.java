 ...
 */
public class CoordinatesLineGeometryHandler extends BaseOuterChainHandler<LineString, org.locationtech.jts.geom.LineString, CoordinateBaseParser, org.geojson.LineString> {

    /**
     * @param theSRSParser
     */
    CoordinatesLineGeometryHandler(@Nonnull(when = NEVER) AbstractGMLBaseSRSParser theSRSParser) {
        super(theSRSParser);
    }
    ...
    public org.locationtech.jts.geom.LineString buildGeometry(GeometryFactory geometryFactory, LineString gmlGeometry, CoordinateBaseParser parser) throws ParserException {
        ...
        if (gmlGeometry.isSetCoordinates()) {
            org.locationtech.jts.geom.LineString lineString = geometryFactory.createLineString(parser.parseCoordinates(gmlGeometry.getCoordinates()));
            this.srsParser.parseSRS(gmlGeometry, lineString);
            return lineString;
        }
        throw new ParserException("It is not possible build a JTS Geometry from this GML LineString : " + gmlGeometry);
    }
    ...
    public org.geojson.LineString buildGeometryAsGeoJson(LineString gmlGeometry, CoordinateBaseParser parser) throws ParserException {
        ...
        if (gmlGeometry.isSetCoordinates()) {
            org.geojson.LineString lineString = new org.geojson.LineString(parser.parseCoordinatesAsGeoJson(gmlGeometry.getCoordinates()));
            lineString.setCrs(this.srsParser.parseSRS(gmlGeometry));
            return lineString;
        }
        throw new ParserException("It is not possible build a GeoJson Geometry from this GML LineString : " + gmlGeometry);
    }
}