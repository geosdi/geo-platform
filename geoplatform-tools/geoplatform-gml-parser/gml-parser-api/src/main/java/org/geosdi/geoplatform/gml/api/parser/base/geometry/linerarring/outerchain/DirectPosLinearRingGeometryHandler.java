 ...
 */
public class DirectPosLinearRingGeometryHandler extends BaseOuterChainHandler<LinearRing, org.locationtech.jts.geom.LinearRing, CoordinateBaseParser, LineString> {

    /**
     * @param theSRSParser
     */
    DirectPosLinearRingGeometryHandler(@Nonnull(when = NEVER) AbstractGMLBaseSRSParser theSRSParser) {
        super(theSRSParser);
        super.setSuccessor(new CoordinatesLinearRingGeometryHandler(this.srsParser));
    }
     ...
     */
    @Override
    public org.locationtech.jts.geom.LinearRing buildGeometry(GeometryFactory geometryFactory, LinearRing gmlGeometry, CoordinateBaseParser parser) throws ParserException {
        ...
        if (gmlGeometry.isSetPosList()) {
            ...
            if ((gmlGeometry.isSetSrsDimension()) && !(directPositionList.isSetSrsDimension()))
                directPositionList.setSrsDimension(gmlGeometry.getSrsDimension());
            org.locationtech.jts.geom.LinearRing linearRing = geometryFactory.createLinearRing(parser.parseCoordinates(directPositionList));
    ...
    public LineString buildGeometryAsGeoJson(LinearRing gmlGeometry, CoordinateBaseParser parser) throws ParserException {
        ...
        if (gmlGeometry.isSetPosList()) {
            ...
            if ((gmlGeometry.isSetSrsDimension()) && !(directPositionList.isSetSrsDimension()))
                directPositionList.setSrsDimension(gmlGeometry.getSrsDimension());
            LineString linearString = new LineString(parser.parseCoordinatesAsGeoJson(directPositionList));
