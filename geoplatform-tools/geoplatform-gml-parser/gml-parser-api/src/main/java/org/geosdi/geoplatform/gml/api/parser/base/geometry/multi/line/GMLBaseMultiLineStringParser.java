 ...
 */
public class GMLBaseMultiLineStringParser
        extends AbstractGMLBaseParser<MultiLineString, MultiLineStringProperty, com.vividsolutions.jts.geom.MultiLineString> {

    private GMLBaseLineStringParser lineStringParser;

    public GMLBaseMultiLineStringParser(GeometryFactory theGeometryFactory,
    ...
    @Override
    protected com.vividsolutions.jts.geom.MultiLineString canParseGeometry(
            MultiLineString gmlGeometry)
            throws ParserException {

        Preconditions.checkState(gmlGeometry.isSetLineStringMember(), "The "
                + "LineStringMember is not present.");

        List<LineString> lines = new ArrayList<LineString>(
                gmlGeometry.getLineStringMember().size());

        for (LineStringProperty lineProperty : gmlGeometry.getLineStringMember()) {
            lines.add(lineStringParser.parseGeometry(
                    lineProperty.getLineString()));
        }
    ...
    @Override
    public com.vividsolutions.jts.geom.MultiLineString parseGeometry(
            MultiLineStringProperty propertyType)
            throws ParserException {

        Preconditions.checkNotNull(propertyType, "The MultiLineString Property "
                + "must not be null.");
