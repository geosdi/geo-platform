 ...
 */
public class GMLBaseMultiPointParser
        extends AbstractGMLBaseParser<MultiPoint, MultiPointProperty, com.vividsolutions.jts.geom.MultiPoint> {
    ...
    public GMLBaseMultiPointParser(GeometryFactory theGeometryFactory,
    ...
    @Override
    protected com.vividsolutions.jts.geom.MultiPoint canParseGeometry(
            MultiPoint gmlGeometry)
            throws ParserException {

        Preconditions.checkArgument(
                !(gmlGeometry.isSetPointMember()) && !(gmlGeometry.isSetPointMembers()),
    ...
    @Override
    public com.vividsolutions.jts.geom.MultiPoint parseGeometry(
            MultiPointProperty propertyType)
            throws ParserException {

        Preconditions.checkNotNull(propertyType, "The MultiPoint Property "
                + "must be not null.");
    ...
    protected interface MemberBuilder {
        ...
        void builMember(MultiPoint gmlGeometry,
                List<Point> points) throws ParserException;
    }

    protected class PointMemberBuilder implements MemberBuilder {

        @Override
        public void builMember(MultiPoint gmlGeometry,
                List<Point> points) throws ParserException {

            if (gmlGeometry.isSetPointMember()) {
                for (PointProperty property : gmlGeometry.getPointMember()) {
                    points.add(pointParser.parseGeometry(property.getPoint()));
                }
            }
        }
    }
