...
public class GMLBasePointParser extends AbstractGMLBaseParser<Point, PointProperty, com.vividsolutions.jts.geom.Point> {
    ...
    public GMLBasePointParser(GeometryFactory theGeometryFactory,
        ...
        Preconditions.checkNotNull(theSrsParser,
                "The GML Coordinate Parser must not be null.");

        this.coordinateParser = theCoordinateBaseParser;
    }

    @Override
    protected com.vividsolutions.jts.geom.Point canParseGeometry(
            Point gmlGeometry)
            throws ParserException {

        return directPos.buildGeometry(geometryFactory, gmlGeometry,
                coordinateParser);
    }

    @Override
    public com.vividsolutions.jts.geom.Point parseGeometry(
            PointProperty propertyType)
            throws ParserException {

        Preconditions.checkNotNull(propertyType,
                "The Property Type must not be null.");
