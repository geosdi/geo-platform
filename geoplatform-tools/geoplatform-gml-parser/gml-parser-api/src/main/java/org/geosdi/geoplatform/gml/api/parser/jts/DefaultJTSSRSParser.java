...
public class DefaultJTSSRSParser extends AbstractJTSSRSParser {
    ...
    @Override
    public void parseSRS(Geometry jtsGeometry, AbstractGeometry gmlGeometry) throws ParserException {
        checkNotNull(gmlGeometry, "GML Geometry must not be null.");
        checkNotNull(jtsGeometry, "JTS Geometry must not be null.");
        if ((jtsGeometry.getUserData() != null) && (jtsGeometry.getUserData() instanceof String)) {
            gmlGeometry.setSrsName((String) jtsGeometry.getUserData());
        } else if (jtsGeometry.getSRID() != 0) {
            gmlGeometry.setSrsName(MessageFormat.format(pattern, jtsGeometry.getSRID()));
        }
    }
}