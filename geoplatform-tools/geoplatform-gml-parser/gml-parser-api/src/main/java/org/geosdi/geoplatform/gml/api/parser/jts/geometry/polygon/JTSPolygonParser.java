...
public class JTSPolygonParser extends AbstractJTSParser<Polygon, PolygonProperty, com.vividsolutions.jts.geom.Polygon> {
    ...
    public JTSPolygonParser(AbstractGMLObjectFactory theGmlObjectFactory,
    ...
    @Override
    protected Polygon canParseGeometry(
        ...
        polygon.setExteriorValue(gmlObjectFactory.createExterior(linearRingParser.parseProperty(
                extertiorRing)));

        for (int i = 0; i < jtsGeometry.getNumInteriorRing(); i++) {
            LinearRing interiorRing = (LinearRing) jtsGeometry.getInteriorRingN(
                    i);
            polygon.addInteriorValue(gmlObjectFactory.createInterior(linearRingParser.parseProperty(
                    interiorRing)));
        }
    ...
    @Override
    public PolygonProperty parseProperty(
            com.vividsolutions.jts.geom.Polygon jtsGeometry)
            throws ParserException {

        Preconditions.checkNotNull(jtsGeometry, "The JTS Polygon Geometry "
                + "must not be null.");
    ...
    @Override
    public JAXBElement<? extends Polygon> buildJAXBElement(
            com.vividsolutions.jts.geom.Polygon geometry) throws ParserException {
