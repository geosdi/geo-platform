...
public abstract class GPAbstractJpaDAO<T extends Object, ID extends Serializable> extends GPCriteriaJpaDAO<T, ID> implements GPBaseJpaDAO<T, ID> {
     ...
     */
    public GPAbstractJpaDAO(@Nonnull(when = NEVER) Class<T> thePersistentClass) {
    ...
    public <S extends T> Collection<S> update(Iterable<T> entities) throws GPDAOException {
        ...
        try {
            return stream(entities.spliterator(), FALSE).filter(Objects::nonNull).map(e -> (S) update(e))
                    .collect(toList());
        } catch (Exception ex) {
            ex.printStackTrace();
            throw new GPDAOException(ex);
    ...
    public Collection<T> persist(Iterable<T> entities) throws GPDAOException {
        checkNotNull(entities != null, "The Parameter entities must not be null.");
        try {
            return stream(entities.spliterator(), FALSE).filter(Objects::nonNull).map(this::persist).collect(toList());
        } catch (Exception ex) {
            ex.printStackTrace();
            throw new GPDAOException(ex);
        }
    ...
    public List<T> findAll(int start, int end) throws GPDAOException {
        try {
            ...
            Root<T> root = criteriaQuery.from(this.persistentClass);
            criteriaQuery.select(root);
            return this.entityManager.createQuery(criteriaQuery).setFirstResult((start == 0) ? 0 : ((start * end)))
