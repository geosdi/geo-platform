...
@Profile(value = "jpa")
public class JpaCarPartDAO extends GPAbstractJpaDAO<CarPart, Long>
        implements ICarPartDAO {

    public JpaCarPartDAO() {
        super(CarPart.class);
    }
    ...
    public CarPart findByPartName(String partName) throws Exception {
        try {
            ...
            Root<CarPart> carPartRoot = criteriaQuery.from(super.getPersistentClass());
            criteriaQuery.select(carPartRoot);
            criteriaQuery.where(super.getCriteriaBuilder().equal(carPartRoot.get("partName"), partName));
