...
public class GPJAXBJakartaContextBuilderTest {

    private static final Logger logger = LoggerFactory.getLogger(GPJAXBJakartaContextBuilderTest.class);
    //
    private static final IGPJAXBContextBuilder GP_JAXB_JAKARTA_CONTEXT_BUILDER = GPJAXBJakartaContextBuilder.jakartaContextBuilder();

    @Test
    public void a_writeCarAsStringTest() throws Exception {
        Car car = new Car();
        car.setModel("AUDI A8");
        car.setPlat("XKTT6846");
        car.setCarParts(createCarParts(25));
        StringWriter writer = new StringWriter();
        GP_JAXB_JAKARTA_CONTEXT_BUILDER.marshal(car, writer);
    ...
    public void e_writeAttributeStoreAsStringTest() throws Exception {
        ...
        StringWriter writer = new StringWriter();
        GP_JAXB_JAKARTA_CONTEXT_BUILDER.marshal(attributeStore, writer);
    ...
    public void f_writeAttributeStoreAsFileTest() throws Exception {
        ...
        GP_JAXB_JAKARTA_CONTEXT_BUILDER.marshal(attributeStore, new File( of(new File(".").getCanonicalPath(), "target", "AttributeStore")
                .collect(joining(separator, "", ".xml"))));
    }
    ...
    public void g_readAttributeStoreFromFileTest() throws Exception {
        logger.info("#######################UNMARSHALL_ATTRIBUTE_STORE_FROM_FILE : {}\n",
                GP_JAXB_JAKARTA_CONTEXT_BUILDER.unmarshal(new File(of(".", "src", "test", "resources", "AttributeStore.xml")
    ...
    public void h_readAttributeStoreFromStringTest() throws Exception {
        logger.info("@@@@@@@@@@@@@@@@@@@@@@@@@@UNMARSHALL_ATTRIBUTE_STORE_FROM_STRING : {}\n",
                GP_JAXB_JAKARTA_CONTEXT_BUILDER.unmarshal(new StringReader("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\n" +
                        "<AttributeStore>\n" +
                        "    <attributes>\n" +
                        "        <attribute>\n" +
