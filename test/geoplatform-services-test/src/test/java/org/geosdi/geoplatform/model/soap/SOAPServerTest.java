...
public class SOAPServerTest extends BaseSoapServiceTest {
    ...
    public void testGetAllServer() throws ResourceNotFoundFault {
        ...
        // Insert new Server
        long idNewServer = this.createAndInsertServer("map.testGetAllServer.com", WMS, organizationTest);
        // Assert of number of Servers
        assertEquals("Total numebr of Servers is wrong after inserted new Server", gpWSClient.getAllServers(organizationNameTest).getServers().size(), totalServers + 1);
        // Delete new Server
        this.deleteServer(idNewServer);
        // Assert of number of Servers
        assertEquals("Total numebr of Servers is wrong after deleted new Server", gpWSClient.getAllServers(organizationNameTest).getServers().size(), totalServers);
    }
    ...
    public void testSaveServer() throws ResourceNotFoundFault, IllegalParameterFault {
        ...
        assertEquals("ServerDTO geoSDI alias is wrong", serverDTO.getAlias(), "geoSDI");
